...
const show = function (e) {
  if (inputTabs[C].value == results[C]) {
    if (e.keyCode == 13) {
      ...
      C === 10
        ? (windowEnd.classList.remove("top1000"),
          clearInterval(intervalStart),
          (yourTime.textContent = "Twój czas to: " + stoper.textContent + "s"))
        : (divFulls[C].classList.remove("remove"), inputTabs[C].focus());
      console.log(stoper.textContent);
      if (stoper.textContent <= 10) {
        youAre.textContent =
          "Ten czas jest wręcz niemożliwy!  Jesteś Geniuszem!";
        ...
        youAre.textContent =
          "Mieścisz się w przeciętności! Jesteś conajmniej średni!";
        lastName.textContent = "conajmniej średni!";
      } else if (stoper.textContent <= 26) {
        youAre.textContent =
          "Matma nie jest Twoją mocną stroną! Jesteś humanistą?";
        lastName.textContent = "humanistą?";
      } else if (stoper.textContent <= 30) {
        youAre.textContent =
          "Słabo, słabo! Jesteś Nienajlepszy, żeby Cie nie obrazić.";
        lastName.textContent = "Nienajlepszy, żeby Cie nie obrazić";
      } else if (stoper.textContent <= 34) {
        youAre.textContent =
          "Mogło być gorzej! A nie.. nie mogło. Jesteś cienias!";
        lastName.textContent = "cienias!";
      } else {
        youAre.textContent =
          "Chyba chodziłeś na wagary zamiast na matme. Jesteś Wagarowiczem!";
